...
const Home: React.FC<HomeProps> = ({ dailyMurli, onSearch }) => {
  ...
  return (
    ...
    >
      ...
      {/* Background Layer - Rotating Rings & White Dot */}
      <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
        <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex items-center justify-center">

          <div className="relative flex items-center justify-center">
            {/* Rays Layer 1 (Clockwise) - Removed gpu-accelerated to prevent transform collision */}
            <div className="absolute w-[200vmax] h-[200vmax] animate-spin-slow opacity-40" style={{ willChange: 'transform' }}>
              <div className="w-full h-full" style={{
                background: 'repeating-conic-gradient(from 0deg at 50% 50%, rgba(255, 255, 255, 0.35) 0deg 4deg, transparent 4deg 12deg)'
              }}></div>
            </div>

            {/* Rays Layer 2 (Counter-Clockwise) - Removed gpu-accelerated */}
            <div className="absolute w-[200vmax] h-[200vmax] animate-spin-slow opacity-25" style={{ animationDirection: 'reverse', animationDuration: '160s', willChange: 'transform' }}>
              <div className="w-full h-full" style={{
      ...
      >
        ...
        <div className="w-full relative bg-white/20 backdrop-blur-xl border border-white/40 rounded-2xl p-4 shadow-[0_4px_20px_rgba(0,0,0,0.1)]">
          ...
          <div className="grid grid-cols-3 gap-2">
            <motion.button
              ...
              className="relative w-full bg-white/10 border border-white/30 rounded-xl px-2 py-3 text-white text-center hover:bg-white/20 transition-colors flex flex-col items-center justify-center min-h-[60px]"
            >
          ...
          <motion.button
            ...
            className="w-full mt-3 bg-white text-[#D32F2F] hover:bg-[#ffebee] transition-colors font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 shadow-lg"
          >
            <FaSearch /> खोजें
          </motion.button>
        </div>
        ...
        <motion.div
          ...
          className="w-full bg-gradient-to-b from-white/25 to-white/10 backdrop-blur-xl rounded-[2rem] p-8 border border-white/30 shadow-[0_15px_50px_rgba(0,0,0,0.25)] text-left relative overflow-hidden group gpu-accelerated"
        >
          ...
          <div className="border-t border-white/20 pt-4 flex flex-col gap-3">
            <motion.button
              ...
              className="w-full py-3.5 bg-white text-[#D32F2F] hover:bg-[#ffebee] transition-colors rounded-xl font-bold font-sans shadow-md flex items-center justify-center gap-2"
            >
        ...
        {activePicker && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-[2rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] w-full max-w-sm overflow-hidden flex flex-col max-h-[70vh]"
            >
              <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-white/95 backdrop-blur-md sticky top-0 z-10">
                <h3 className="text-lg font-bold text-gray-800 font-hindi-title">
                  {activePicker === 'day' && 'दिन चुनें'}
                  {activePicker === 'month' && 'माह चुनें'}
                  {activePicker === 'year' && 'वर्ष चुनें'}
                </h3>
                <button onClick={closePicker} className="text-gray-400 hover:text-[#D32F2F] p-1.5 rounded-full hover:bg-gray-100 transition-colors">
                  <FaTimes size={18} />
                </button>
              </div>
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {days.map(d => (
                      <button
                        ...
                        className={`aspect-square flex items-center justify-center rounded-xl font-bold transition-all text-sm font-sans ${day === d.toString()
                          ? 'bg-[#D32F2F] text-white shadow-lg shadow-red-200 transform scale-105'
                          : 'bg-gray-50 text-gray-700 hover:bg-[#ffebee] hover:text-[#D32F2F] active:scale-95'
                          }`}
                      >
                        {d}
                      </button>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {months.map(m => (
                      <button
                        ...
                        className={`p-4 rounded-2xl font-bold transition-all text-left flex flex-col items-start ${month === m.value
                          ? 'bg-[#D32F2F] text-white shadow-lg shadow-red-200 transform scale-[1.02]'
                          : 'bg-gray-50 text-gray-700 hover:bg-[#ffebee] hover:text-[#D32F2F] active:scale-95'
                          }`}
                      >
                        <span className={`text-xs mb-1 font-sans ${month === m.value ? 'text-white/80' : 'text-gray-500'}`}>{m.value}</span>
                        <span className="text-lg leading-none font-hindi-title tracking-wide">{m.label}</span>
                      </button>
                    ))}
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {years.map(y => (
                      <button
                        ...
                        className={`py-3 rounded-xl font-bold transition-all text-sm font-sans ${year === y.toString()
                          ? 'bg-[#D32F2F] text-white shadow-lg shadow-red-200 transform scale-105'
                          : 'bg-gray-50 text-gray-700 hover:bg-[#ffebee] hover:text-[#D32F2F] active:scale-95'
                          }`}
                      >
